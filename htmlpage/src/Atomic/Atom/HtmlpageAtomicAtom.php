...
class HtmlpageAtomicAtom extends HtmlpageAtomic {
      ...
      $output .= '<ul class="dropdown-menu" aria-labelledby="tile-meeting-share-link" class="color-00a9e0 bg-ffffff">';
        $output .= '<li>';
          $output .= '<span class="margin-left-12">';
            $output .= '<a href="mailto:?subject=Share Link&amp;body=' . $this->getHtmlGuestShareLink($meeting_nid) . '" class="color-000 text-decoration-none">';
              $output .= 'Share Link';
            $output .= '</a>';
          $output .= '</span>';
        $output .= '</li>';
        $output .= '<li>';
          $output .= '<span class="margin-left-12">';
            $output .= '<a href="' . $this->getPdfDownloadLink($meeting_nid) . '" class="color-000 text-decoration-none">';
              $output .= 'Download PDF';
            $output .= '</a>';
          $output .= '</span>';
        $output .= '</li>';
   ...
   *
   */
  public function getHtmlGuestShareLink($meeting_nid = NULL) {
    $output = NULL;
   ...
   *
   */
  public function getPdfDownloadLink($meeting_nid = NULL) {
    $app_root = \Drupal::hasService('app.root') ? \Drupal::root() : DRUPAL_ROOT;
    ...
    $pdf_link .= 'genpdf/meeting/page/' . $meeting_nid;
